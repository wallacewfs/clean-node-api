...
describe('DbAddAccount Usecase', () => {
  ...
  test('Should return true if loadAddcountByEmailRepository on returns null', async () => {
    const { sut } = makeSut()
    const isValid = await sut.add(mockAddAccountParams())
    expect(isValid).toBe(true)
  })

  test('Should return false if loadAddcountByEmailRepository not returns null', async () => {
